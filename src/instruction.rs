...
impl Instruction {
    ...
    pub fn new(opcode: Opcode, address: usize) -> Self {
        Self { opcode, address, operand: None }
    }
    ...
    pub fn new_with_operand(opcode: Opcode, address: usize, operand: Operand) -> Self {
        Self { opcode, address, operand: Some(operand) }
    }
...
mod tests {
    ...
    fn instruction_to_string() {
        ...
        let instruction = Instruction::new_with_operand(Opcode::PushString, 0, Operand::new_string("Hello, world!"));
        assert_eq!(instruction.to_string(), "PushString Hello, world!");

        let instruction = Instruction::new_with_operand(Opcode::PushNumber, 0, Operand::new_float("3.14"));
