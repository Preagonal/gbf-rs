use gbf_rs::{basic_block::BasicBlockType, function::{Function, FunctionId}, instruction::Instruction, opcode::Opcode, operand::Operand};
...
fn main() {
    ...
    block.add_instruction(Instruction::new_with_operand(Opcode::Jeq, 3, Operand::new_int(8)));

    // block 2
    let block2_id = fun.create_block(BasicBlockType::Exit).unwrap();
    let block2 = fun.get_block_mut(block2_id).unwrap();
    block2.add_instruction(Instruction::new_with_operand(Opcode::PushNumber, 4, Operand::new_int(42)));
